...
predictions = a3;
multiclass_y = (1 : num_labels) == y;
reg = (lambda/(2*m)) * (sum(sum((Theta1(:,2:end)).^2)) + sum(sum((Theta2(:,2:end)).^2)));
J = ((1/m)* (sum(sum((-multiclass_y).* log(predictions) - (1-multiclass_y).* log(1.-predictions))))) + reg;
...
%
  ...
  DELTA3 = a3 - multiclass_y; 
  DELTA2 = (DELTA3 * Theta2(:,2:end)) .*sigmoidGradient(z2).*(1 - sigmoidGradient(z2)) ;
